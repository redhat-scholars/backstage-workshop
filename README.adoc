...
```
5. Configure your new GitHub OAuth App's `id` and `secret`, and add your gh org to the list of approved orgs:
+
```bash
...
7. Install the xref:#install-the-namespace-configuration-operator[Namespace Configuration Operator] (see below)
...
```
9. Share access by inviting users to your github org. Users will need to accept the invite before accessing the cluster:
+
```bash
...
=== Install the Namespace Configuration Operator
