...
IDPs help teams curate, manage and replicate these environments. _Backstage_ is an open source CNCF project for building developer platforms, and for encapsulating tools, services, documentation and best practices in “golden paths” to ease onboarding and daily development. _Janus_ is Red Hat's community project for contributing to the Backstage core and the ecosystem around it. The IDP presented in this workshop is referred to by both names: fundamental Backstage features are described as such, while you'll work with a running Janus instance labeled with that name.
