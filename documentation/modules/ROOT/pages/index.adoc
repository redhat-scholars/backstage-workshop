...
In these exercises you will use Backstage _Templates_ to deploy the *POI Map* application, a microservices-based application showing geo-data points into a world map. The architecture is divided in three application tiers as you can see from the picture below. 
...
In the xref:01-setup.adoc[first section], you'll sign in to OpenShift and to Backstage with your GitHub ID. Throughout the exercises, you'll note that your GitHub ID is your user name in both OpenShift and Backstage, and that your application components are deployed to an OpenShift _Project_ with the same name. Both your user name and your Project name will match the GitHub ID you use to authenticate.

In xref:02-templates.adoc[section 2], you'll get to know Backstage Templates as you use two provided Templates to fork, build, and deploy the graphical map interface frontend and the API gateway that supports it.
