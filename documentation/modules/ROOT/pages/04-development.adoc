...
IDP productivity is achieved by aligning platform services with developer experiences.  This final chapter in our Golden Path workshop involves using OpenShift Dev Spaces to iterate on our three-tier example application.

[#devspaces]
== OpenShift Dev Spaces

The goal of this chapter will be to add a custom datapoint to the "Red Hat Summit POI" dataset, which is stored in-memory within the Gateway component.

You will use Dev Spaces to introduce a new API endpoint to your Gateway component.  

While there are many ways to craft a Golden Path for your development staff, including OpenShift Dev Spaces provides a quick path to success with zero laptop installation effort.
...
Click on "Allow selected permissions" to grant Dev Spaces access to your OpenShift account

image::auth_6.png[]

IMPORTANT: Make sure to click on *`Grant`* next to the **Summit23JanusN** GitHub Org before approving Dev Spaces access (where "N" matches your cluster's GitHub Org ID)
...
Open `/src/main/java/com/rhdevelopers/summit/workshop/SummitBackendResource.java` and uncomment the following code block from line 72 to 81:
...
You will find this block of code near the bottom of the file:

image::uncomment-editor.png[]

### Commit your change and sync

Click on the Source Control tab in the side menu, then select the plus icon to add your change to the changeset. 
